...
## 2026-06-18 · 修复新建角色素材选择

- 修复新建角色弹窗里「选择文件」无响应的问题：
  - 将静态 dropzone 改为带隐藏 file input 的可点击/可拖拽 label。
  - 支持 `.txt` / `.md` 文件读取，并在弹窗内显示已选择文件名。
  - 增加粘贴文本输入区，与“也可直接粘贴文本”的文案一致。
  - 如果新建角色时已经提供素材，会直接创建角色并进入训练；未提供素材时仍沿用创建后进入上传弹窗的流程。
...
- 完成会话管理回归：
  - 会话新建、重命名、列表确认、删除测试通过。
  - 历史抽屉中问答/自动台词分段切换、新建、改名、删除确认和关闭流程测试通过。
- 完成复制/下载回归：
  - 问答 assistant 气泡复制、自动台词卡片复制、Header「复制全部」测试通过。
  - 问答 `.txt` 下载和自动台词 `.txt` 下载测试通过，文件内容包含角色、模式、导出时间和分段内容。
- 完成移动端抽屉回归：
  - 手机尺寸下左栏菜单打开、遮罩关闭、选择角色自动关闭测试通过。
  - 小屏历史抽屉滚动、分段切换、重命名输入、删除确认、完成关闭测试通过。
